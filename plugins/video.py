...
async def stream(client, m: Message):
    ...
    if not media and not ' ' in m.text:
        await msg.edit("❗ __Send Me An Live Stream Link To Start Video Streaming!__")

    elif ' ' in m.text:
        text = m.text.split(' ', 1)
        query = text[1]
        if not 'http' in query:
            return await msg.edit("❗ __Send Me An Live Stream Link To Start Video Streaming!__")
        ...
        if match:
            ...
            link = query
            thumb = "https://telegra.ph/file/68db740c839de49b33c6f.jpg"
