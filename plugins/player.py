...
async def add_to_playlist(_, message: Message):
    ...
    if message.reply_to_message and message.reply_to_message.video:
        ...
        if message.reply_to_message:
            ...
            k=await message.reply_text("❗ __**Send Me An Video To Play In Telegram Video Chat !**__")
            await delete(k)
            return
...
async def stream(client, m: Message):
    if m.reply_to_message:
        ...
        k=await m.reply_text("❗ __**Send Me An Live Stream Link To Start Live Streaming !**__")
        await delete(k)
        return
    regex = r"^(?:https?:\/\/)?(?:www\.)?youtu\.?be(?:\.com)?\/?.*(?:watch|embed)?(?:.*v=|v\/|\/)([\w\-_]+)\&?"
    match = re.match(regex,link)
    ...
    if k == False:
        ...
        await delete(s)
        return
    s=await m.reply_text(f"▶️ **Started Streaming!**", disable_web_page_preview=True)
    await delete(s)
